var _polaroidGallery = (function () {
    ...
    // var url = "/product/tao-girl-detail-data.html?id=6";

    // return ;
    function _polaroidGallery(url) {

        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                var myArr = JSON.parse(xmlhttp.responseText);
                if(myArr.data.imgList.length==0){
                    document.getElementById("gallery").innerHTML = "暂时还没有照片哦";
                    return false;
                }
                observe();
                setGallery(myArr.data.imgList, myArr.data.realName);
